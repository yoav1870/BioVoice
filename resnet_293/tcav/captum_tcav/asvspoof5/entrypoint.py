...
target_classes = (
    ["spoof", "bonafide"]
    if config.target_class_mode == "both"
    else [config.example_class]
)
...
for system_id in config.system_ids:
    ...
    shared_speakers = (
        shared_speakers_for_both_classes(config, system_id, manifest)
        if config.target_class_mode == "both"
        else None
    )
    ...
    for target_class in target_classes:
        subset = (
            ...
            if config.target_class_mode == "both"
            else select_subset_for_class(config, system_id, manifest, target_class)
        )
        ...
        for speaker_id in subset.selected_speakers:
            ...
            print(
                f"Processing system={system_id} target={target_class} speaker={speaker_id} "
        ...
        if config.output_mode == "mean":
            ...
            for row in csv_rows:
                ...
                system_totals[concept_name]["magnitude"] = float(
                    system_totals[concept_name]["magnitude"]
                ) + float(row["magnitude"])
                system_totals[concept_name]["sign_count"] = float(
                    system_totals[concept_name]["sign_count"]
                ) + float(row["sign_count"])
                ...
                system_rows.append(
                    {
                        ...
                        "magnitude": float(
                            format(float(row["magnitude"]) / num_speakers, "g")
                        ),
                        "sign_count": float(
                            format(float(row["sign_count"]) / num_speakers, "g")
                        ),
                        "concept_name": concept_name,
                        "target_class": row["target_class"],
                    }
        ...
        csv_path = write_scores_csv(
            ...
            / f"tcav_{source_partition}_{system_id}_{target_class}.csv",
        ...
        subset_summary = {
            ...
            "split": config.split_name,
            "target_class": target_class,
            "target_class_mode": config.target_class_mode,
            "model_loading_mode": config.model_loading_mode,
            ...
            "shared_speakers": shared_speakers if shared_speakers is not None else [],
        ...
        (
            OUTPUT_DIR / f"subset_{source_partition}_{system_id}_{target_class}.json"
        ).write_text(
            json.dumps(subset_summary, indent=2),
            encoding="utf-8",
        )

        print(f"Completed {system_id} target={target_class}. CSV path: {csv_path}")
