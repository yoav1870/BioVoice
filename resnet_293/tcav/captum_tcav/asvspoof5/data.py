...
def validate_config(config: Config) -> None:
    ...
    if not config.wespeaker_model_dir.is_dir():
        raise FileNotFoundError(f"Missing wespeaker model dir: {config.wespeaker_model_dir}")
    if not config.wespeaker_models_dir.is_dir():
        raise FileNotFoundError(f"Missing wespeaker models dir: {config.wespeaker_models_dir}")
...
def load_input(
    ...
    """Load audio from tar, compute kaldi fbank, normalise frames, return (B, T, F)."""
    target_frames = infer_target_frames(config.concept_root, config.concept_names)
    fbank_batch: list[torch.Tensor] = []
    for utt_id in utt_ids:
        ...
        if sample_rate != config.fbank_sample_frequency:
            waveform = torchaudio.functional.resample(
                waveform, sample_rate, config.fbank_sample_frequency
            )
        waveform = waveform[:1, :].float()
        # WeSpeaker convention: scale waveform by 2^15.
        waveform = waveform * (1 << 15)

        feats = kaldi.fbank(
            waveform,
            num_mel_bins=config.fbank_num_mel_bins,
            frame_length=config.fbank_frame_length,
            frame_shift=config.fbank_frame_shift,
            dither=0.0,
            sample_frequency=config.fbank_sample_frequency,
        )  # (T, F)

        # Normalise frames: transpose to (F, T) so normalize_frames works on
        # the last dimension (T), then transpose back to (T, F).
        feats_ft = feats.permute(1, 0)                          # (F, T)
        feats_ft = normalize_frames(feats_ft, target_frames)    # adjust T
        feats_tf = feats_ft.permute(1, 0)                       # (T, F)
        fbank_batch.append(feats_tf)

    return torch.stack(fbank_batch)  # (B, T, F)
