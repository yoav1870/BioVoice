...
OUTPUT_BASE_DIR = Path(__file__).resolve().parent.parent / "not_our_positive_concepts_dataset"
...
def generate_concept_patch_for_raw_input(mean_length: float, std_length: float, mean_tone_degree: float, std_tone_degree: float, mean_thickness: float, rng: Optional[np.random.Generator] = None, random_thickness_boundary: float=0.5) -> np.ndarray:
    ...
    Returns:
        2D ndarray in float32 with values normalized to [0, 1].
    """
    ...
    _draw_line_gaussian(S, int(t0), int(L), float(y0), float(k), float(fwhm))
    S -= S.min()
    if S.max() > 0:
        S /= S.max()
...
def generate_concept_patch_for_labels(
    ...
    Args:
        length: "short" or "long" duration label.
        tone: "rising", "dropping", or "constant" slope type.
        rate: "steep" or "flat" slope angle (ignored if tone=="constant").
        thickness: "thin" or "thick" Gaussian line thickness.
...
def create_concept_dir(
    ...
    Args:
        base_dir: Root directory for saving.
    ...
    concept_name = _dir_name(length, tone, rate, thickness)
